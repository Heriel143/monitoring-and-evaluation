        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-6">
                    <div class="card">
                        <div class="card-body">
                            <h1 class="text-xl font-bold card-title">Edit Indicator</h1>
                            @if (count($errors))
                                @foreach ($errors->all() as $error)
                                    <p class="alert alert-danger">{{ $error }}</p>
                                @endforeach
                            @endif
                            <form action="{{ route('indicator.update') }}" method="post" class="mt-4" id="myForm">
                                @csrf
                                ...
                                <div class="mb-3 row">
                                    ...
                                    <div class="col-sm-9 form-group">
                                        <input class="form-control" value="{{ $indicator->indicator_number }}"
                                            type="text" name="indicator_number" id="name">
                                    </div>
                                </div>
                                <div class="mb-3 row">
                                    <label for="example-text-input" class="col-sm-3 col-form-label">Name</label>
                                    <div class="col-sm-9 form-group">
                                        <input class="form-control" type="text" value="{{ $indicator->title }}"
                                            name="title" id="name">
                                    </div>
                                </div>
                                <div class="mb-3 row">
                                    ...
                                    <div class="col-sm-9">
                                        <select name="disagretion" class="form-select " aria-label="Default select example"
                                            ...
                                            @foreach ($disagretions as $disagretion)
                                                <option
                                                    value="{{ $disagretion->name }}"{{ $disagretion->name == $indicator->disagretion ? 'selected' : '' }}>
                                                    {{ $disagretion->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="mb-3 row">
                                    ...
                                    <div class="col-sm-9 form-group">
                                        <select name="unit" class="form-select " aria-label="Default select example"
                                            ...
                                            <option selected=""></option>
                                            {{-- @foreach ($disagretions as $disagretion) --}}
                                            <option value="number"{{ $indicator->unit == 'number' ? 'selected' : '' }}>
                                                Number
                                            </option>
                                            <option value="percent"{{ $indicator->unit == 'percent' ? 'selected' : '' }}>
                                                Percent
                                            </option>
                                            {{-- @endforeach --}}
                                        </select>
                                    ...
                                    @foreach ($indicators as $key => $indicator)
                                        <tr>
                                            ...
                                            <td class="flex justify-around">
                                                <a href="{{ route('edit.indicator',$indicator->id) }}" class="btn btn-info sm" title="Edit Data"> <i
                                                        class="fas fa-edit"></i>
                                                </a>

                                                <a href="{{ route('delete.indicator',$indicator->id) }}" class="btn btn-danger sm"
